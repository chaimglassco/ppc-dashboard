...
The selected-range sales request now uses `group_by=day`; its normal ASIN totals continue to populate ranking while `Daily` feeds the quick-stats chart. After promotion, apply a range containing several completed days and reconcile Spend, PPC Sales, Total Sales, calculated ACOS, and calculated TACOS. Verify all five colored, individually normalized lines and their circular markers across desktop and mobile widths, then hover the first, middle, and last dates to confirm each popup stays within the chart and reports all five actual metrics. Switching emphasis or hovering chart metrics is client-only and must not create another Scale Insights request. No environment or storage migration is required.
