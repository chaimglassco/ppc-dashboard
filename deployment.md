...
Deploy the feature with both `/ppc/api/dashboard/untargeted-opportunities` and its dashboard card. After promotion, sign in through Pipeline, select a product and week, then click **Load Opportunities**. Confirm the response is `200`, its period matches the card, every displayed row has converting Sales/Orders, and Scale Insights exact coverage reports each row as uncovered. Change all four criteria controls and confirm no additional endpoint request occurs. Confirm **Fetch Again** remains visible even when the criteria produce zero matches and makes one new request. Then click the shared **Refresh Data** button and confirm one opportunity reload occurs because the card was previously requested.
