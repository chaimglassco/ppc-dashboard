...
The selected-range sales request now uses `group_by=day`; its normal ASIN totals continue to populate ranking while `Daily` feeds the quick-stats chart. After promotion, apply a range containing several completed days and reconcile Spend, PPC Sales, Total Sales, calculated ACOS, and calculated TACOS. Switching chart metrics is client-only and must not create another Scale Insights request. No environment or storage migration is required.
