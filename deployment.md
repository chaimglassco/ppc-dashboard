...
The third-panel redesign is presentation-only and requires no environment, API, or storage migration. After deployment, verify Weekly Goals and Budget Utilization lead the workspace, Weekly Performance Metrics appears immediately below them, active-goal and pacing badges render correctly, and the responsive order remains Goals → Budget → Metrics. Recheck all existing goal, budget-history, Target ACOS, metric refresh, notes, and action controls.
