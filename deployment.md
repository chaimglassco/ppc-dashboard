...
The first rollout is a diagnostic gate. After approval and deployment, select a product with an ASIN and trigger one Retry. Use the returned Reference ID to locate `campaign_tool_contract` plus either `campaign_capability_missing` or `campaign_rows_unreadable` in Vercel runtime logs. Confirm logs contain only matched dates, marketplace, read-only tool/schema names, nonsensitive schema choices, capability fields, and bounded response structure. They must not contain the ASIN, campaign names/IDs, metric values, user data, tokens, or raw MCP content. If campaign grouping is not advertised, confirm no advertising request was made. Record the exact live contract and implement an explicit adapter; never add a guessed grouping argument or aggregate fallback.

For the final provider rollout, confirm the server makes one campaign-scoped request for the immediately preceding matched period, campaign rows are sorted by Previous Week Spend, the first ten and Show all work, and every Current Week Spend cell contains a pending dash. Open several campaign names and confirm they reach the matching previous-period Scale Insights campaign trend without OAuth or connector material in the URL. Test Refresh Data, an ungranted user, an empty ASIN/week, malformed provider data, pagination warnings, and both stable provider error codes; all responses must remain no-store. Do not call the feature shipped until an authenticated production request returns `200` and the campaign Spend total reconciles with Scale Insights for the same ASIN and dates. Current-period retrieval and deltas remain deferred.
