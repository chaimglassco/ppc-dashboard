...
The assistant route calls `openai/gpt-5.4-mini` through Vercel AI Gateway and opens the existing Scale Insights MCP connector with the same verified Pipeline user subject. Do not expose `AI_GATEWAY_API_KEY`, Vercel OIDC, or MCP access tokens as `NEXT_PUBLIC_` values or place them in browser storage. Vercel deployments should use the automatically available project OIDC token. For local development outside `vercel dev`, copy `.env.example` to `.env.local` and set a scoped AI Gateway key; the Scale Insights connector still requires project OIDC and per-user hosted consent. If a pulled OIDC token expires, refresh it with `vercel env pull .env.local --yes` and restart the dev server.
