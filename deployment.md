...
The diagnostic gate was deployed at commit `c68cdc9` as production deployment `dpl_AiPseuCo3En2hqSbrT1W8xTAg3fJ`. Authenticated Reference ID `7274368f-fc45-4e62-ab36-cd7d799273f1` produced the expected sanitized `campaign_tool_contract` and `campaign_capability_missing` events. The live schema exposes no campaign grouping input or dedicated campaign-reporting tool, and the adapter made no aggregate advertising request. Keep this gate in place; never add a guessed grouping argument or aggregate fallback.

If Scale Insights later adds campaign reporting, repeat the contract capture before adapting the request. For that future rollout, confirm the server makes one campaign-scoped request for the immediately preceding matched period, campaign rows are sorted by Previous Week Spend, the first ten and Show all work, and every Current Week Spend cell contains a pending dash. Open several campaign names and confirm they reach the matching previous-period Scale Insights campaign trend without OAuth or connector material in the URL. Do not call the table operational until an authenticated production request returns `200` and its Spend reconciles with Scale Insights for the same ASIN and dates.
