...
After deployment, select a product with an ASIN and verify the staged campaign section appears between the paired weekly summaries and action items. Confirm the server makes one `get_ads_performance` request for the immediately preceding matched period, campaign rows are sorted by Previous Week Spend, the first ten and Show all work, and every Current Week Spend cell contains a pending dash. Open several campaign names and confirm they reach the matching previous-period Scale Insights campaign trend without OAuth or connector material in the URL. Test Refresh Data, an ungranted user, an empty ASIN/week, and a provider failure; successful and error responses must remain no-store. A pagination limitation must surface as a warning rather than silently implying completeness. Do not add current-period retrieval or deltas until the previous Spend rows are confirmed with the live MCP account.
