...
The third-panel redesign is presentation-only and requires no environment, API, or storage migration. After deployment, verify the visibly shaded `Goals & Budget` control surface contains Weekly Goals and Budget Utilization, Weekly Performance Metrics appears as a separate full-width card immediately below, active-goal and pacing badges render correctly, and the responsive order remains Goals → Budget → Metrics. Confirm dark card caps/icon tiles, accented goal rows, two-column budget tiles, and black primary controls are present, then recheck all existing goal, budget-history, Target ACOS, metric refresh, notes, and action controls.
