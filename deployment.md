...
Deploy the feature with both `/ppc/api/dashboard/untargeted-opportunities` and its dashboard card. After promotion, sign in through Pipeline, select a product and week, then click **Load Opportunities**. Confirm the response is `200`, its period matches the card, zero-sales rows remain visible when criteria are blank, all six requested metrics reconcile with Scale Insights, and exact coverage reports each row as uncovered. Confirm the coverage request uses the live `query_list` batch field. Change all four criteria controls and confirm no additional endpoint request occurs. Confirm **Fetch Again** remains visible even when the criteria produce zero matches and makes one new request. Then click the shared **Refresh Data** button and confirm one opportunity reload occurs because the card was previously requested.
