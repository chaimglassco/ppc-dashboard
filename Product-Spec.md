...
Directly below Campaign Week-over-Week Comparison, an on-demand card helps the PPC operator find all customer search terms and product ASINs that do not yet have exact PPC coverage, including those without Sales or Orders. The operator can filter the loaded list by opportunity type, minimum PPC Sales, minimum PPC Orders, and maximum ACOS. Blank criteria mean no metric restriction. Results are ordered by PPC Sales, show ten rows initially, and can expand with Show all.

Every displayed result shows Search Term, Impressions, Clicks, Spend, Sales, Orders, ACOS, and a **Not targeted** badge. Product ASIN values open their Amazon detail page. The selected week is `Final` when complete and `Partial` when capped at yesterday. Empty, authorization, unavailable-capability, malformed-response, loading, retry, and partial-data states remain explicit. Loading is user-initiated to control MCP usage, filter changes are local, and **Fetch Again** stays available after a successful load regardless of the filtered row count.
