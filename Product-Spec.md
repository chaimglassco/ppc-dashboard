...
The PPC route starts in the existing Products workspace and exposes Dashboard and Products as keyboard-accessible switch tabs above the three-panel layout. Dashboard opens a full-width monochrome performance overview based on the supplied reference. Its temporal ledger shows Today, Yesterday, 7 Days, 14 Days, and 30 Days. The weekly windows aggregate available browser-local product reports; unsupported daily windows are explicitly unavailable. The ASIN ranking uses the current saved week with Spend, Sales, Orders, ACOS, TACOS, account shares, and prior-week Sales momentum. Keyword, campaign, product-target, and search-term ledgers render their complete table scaffolds with explicit pending-source states for later refinement. Every Dashboard data section is a closed-by-default disclosure. A shared search matches ASIN, SKU, or product name and recalculates all timeframe cards, rankings, coverage counts, and disclosure scope labels. Switching views or filtering creates no new network request or persisted preference.
