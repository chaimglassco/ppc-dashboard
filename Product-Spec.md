...
Directly below Campaign Week-over-Week Comparison, a card helps the PPC operator find converting PPC search terms and product ASINs that do not yet have exact PPC coverage. The server includes only rows with at least one attributed order. The operator can filter the saved list by opportunity type, minimum PPC Sales, and maximum ACOS. Blank editable criteria mean no additional metric restriction. Impressions, Clicks, Spend, Sales, Orders, and ACOS headers sort highest-to-lowest or lowest-to-highest without another provider request. The summary displays filtered Spend, Sales, Orders, and weighted ACOS totals. Each row links to its ASIN-, period-, and search-term-scoped Scale Insights source report. Results default to PPC Sales descending, show ten rows initially, and can expand with Show all.
