...
Directly below Campaign Week-over-Week Comparison, a card helps the PPC operator find converting PPC search terms and product ASINs that do not yet have exact PPC coverage. The server includes only rows with at least one attributed order. The operator can filter the saved list by opportunity type, minimum PPC Sales, and maximum ACOS. Blank editable criteria mean no additional metric restriction. Results are ordered by PPC Sales, show ten rows initially, and can expand with Show all.
