...
The previous-week comparison layer overlays the matching validated performance snapshot on the prior local report, then passes those metrics to the existing metric renderer. Display formatting rounds and adds thousands separators without modifying cached precision. The renderer derives a percentage delta when the prior value is nonzero and classifies favorable direction by metric: lower Spend/ACOS/TACOS is favorable, while higher sales/orders is favorable. A newly created report copies the preceding report's `previousWeekResult`; the renderer also falls back to that preceding value for older blank reports, while any text already stored on the selected week wins. Large grouped cards, comparison badges/footer rows, budget alignment, and removal of the redundant summary are presentation-only.
