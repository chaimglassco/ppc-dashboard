...
The staged server adapter makes one raw ASIN-scoped `get_ads_performance` request for the previous matched period. It discovers an advertised campaign grouping field and follows advertised cursor, offset, or page pagination up to bounded limits. Grouping discovery supports named scope/level fields and schema choices expressed with either `enum` or `const`. Provider results must match the requested country and exact previous date range; only campaign name and nonnegative Spend are required at this stage. Campaign ID and sponsored type remain nullable when the provider omits them. Rows normalize from flat records, nested campaign/entity records, structured JSON, or an MCP text table paired with structured totals, and duplicates aggregate by ID/type when available or normalized campaign name otherwise. The current period is represented only by its dates and is not requested. Raw MCP content and credentials never cross the server boundary.
