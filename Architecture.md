...
Active goals may carry validated `metric` and `unit` planning metadata. Their Actual value is a render-time projection of the selected validated Scale Insights snapshot and is never user-editable. Full-period completeness is derived by comparing the snapshot end date with the selected Wednesday plus six days. Goal resolution copies the derived display value and optional Partial/Final state into history, preserving what the user evaluated. Recognizable legacy titles are mapped to supported metrics; unrecognized titles remain intact until the user selects a structured goal.
