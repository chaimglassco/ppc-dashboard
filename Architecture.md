...
The Library adapter adds a final-active-document preflight to `document.delete`, matching the disabled state in each document card. Recovery opens even when the normal tombstone count is zero and separately loads protected permanent-deletion history. The local `/ppc/api/library/recovery/purged` route is ADMIN-only and treats the immutable legacy artifact plus the newest matching pre-purge Pipeline backup as read-only sources. It exposes an explicit restore action only for “Monitor Product Listing Prices Through BQool”; the action recreates the original document identity and does not restore any other intentionally purged document.
