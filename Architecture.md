...
PPC Conversion Rate derives only from optional exact `ppcClicks` and PPC Orders. The provider adapter requests one raw ASIN advertising row, checks normalized exact-click aliases in that row, advertising totals, and the sales summary, and accepts a row value only when it is unambiguous and scoped to the requested ASIN. This replaces the manual click field without adding another MCP request. It does not use Total Sessions or search-term totals as a fallback. `totalSessions` remains readable for backward compatibility but no longer drives the conversion card.
...
The staged server adapter makes one raw ASIN-scoped `get_ads_performance` request for the previous matched period only after its live schema explicitly advertises a campaign grouping field. It never falls back to an aggregate request. It follows advertised cursor, offset, or page pagination up to bounded limits. Grouping discovery supports named scope/level fields and schema choices expressed with either `enum` or `const`. Provider results must match the requested country and exact previous date range; only campaign name and nonnegative Spend are required at this stage. Campaign ID and sponsored type remain nullable when the provider omits them. Rows normalize from flat records, nested campaign/entity records, structured JSON, or an MCP text table paired with structured totals, and duplicates aggregate by ID/type when available or normalized campaign name otherwise. The current period is represented only by its dates and is not requested.

Production contract inspection on deployment `dpl_AiPseuCo3En2hqSbrT1W8xTAg3fJ` confirmed only ASIN/date/ad-type/paging/sorting/summary inputs on `get_ads_performance`; campaign grouping is absent, and the advertised read-only tool inventory has no dedicated campaign report. The boundary therefore returns `campaign_capability_missing` before provider execution. This is the supported terminal state for the current connector rather than a retryable transport failure.

Every campaign attempt receives a random request ID. The server logs the matched dates, marketplace, read-only tool names, advertising input property names and nonsensitive choices, detected grouping/pagination fields, and—only when rows cannot be decoded—a bounded structural response fingerprint. Dynamic object keys, scalar values, campaign names/IDs, ASINs, user identity, tokens, and raw MCP content are excluded. The browser receives only the safe error code, message, and correlation ID.

`CampaignWeeklyComparison` is a focused Client Component below the paired summaries. It validates the staged Spend-baseline DTO, cancels stale product/week requests, and keeps results in an in-memory map keyed by country, ASIN, and Wednesday. The shared Refresh Data version invalidates the active entry. The table sorts by previous Spend, shows ten rows before Show all, and renders an explicit pending dash in every Current Week Spend cell. Trend destinations use only the fixed Scale Insights origin plus validated server-returned campaign ID/type and previous dates; they open with `noopener noreferrer`. No baseline data enters `glassco.ppcPerformanceNotes.v1`, `glassco.ppcPerformanceCache.v1`, or a new storage key.
