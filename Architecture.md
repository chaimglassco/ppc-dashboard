...
Pure domain rules classify every joined campaign exactly once. Lost current-week Sales and newly inefficient Spend (previous Spend zero, current Spend positive, and either zero Sales or ACOS at least 15%) run before the general direction matrix. The remaining strict Spend/Sales increases and decreases populate four Good/Bad categories; unchanged or unmatched movement goes to Neutral. Current ACOS derives from current Spend divided by current Sales. Category rows sort by absolute Sales change, then Spend change and campaign name.
