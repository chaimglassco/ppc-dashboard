...
The PPC route owns transient Dashboard/Products tab state. Products retains the existing three-panel editor. Dashboard renders a full-width `PerformanceOverviewDashboard` and derives 7-, 14-, and 30-day summaries plus current-week ASIN rankings from already validated `WeeklyPpcReport` records. It does not fetch or persist data. Today and Yesterday remain unavailable until a daily source exists, and detailed keyword/campaign/product-target/search-term matrices fail visibly into source-pending rows instead of presenting sample HTML values as account data. A transient ASIN/SKU/product-name filter narrows the product set before every aggregation, keeping cards, ranking, latest-update metadata, coverage summaries, and pending-detail scope consistent. Native `details` elements provide independent closed-by-default section state without storage.
