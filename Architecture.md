...
The dashboard performance effect requests only the selected product ASIN, fixed `US` marketplace, and selected reporting-week start. A successful validated response replaces and locks the five imported source metrics for that product/week, while the calculated values remain read-only. Manual source entry remains available when Scale Insights is unconfigured or temporarily unavailable. Imported values are copied into the existing versioned local report draft, but Scale Insights credentials and raw MCP responses are not stored.
