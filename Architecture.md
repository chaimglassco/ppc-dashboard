...
The client validates the response before rendering it. Campaign, keyword target, product target, and search-term rows share a normalized read-only metrics contract. ACOS, ROAS, CTR, CPC, and conversion rate are derived from returned totals. The date and ASIN controls are transient UI state; no report, credential, or custom range is added to browser storage. Local saved weekly reports remain a visible fallback for temporal totals and the ASIN ranking.
