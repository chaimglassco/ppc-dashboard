...
The server discovers the connected `get_ads_performance` input schema, applies an advertised campaign grouping field when available, and follows advertised cursor, offset, or page pagination up to bounded request and campaign limits. Grouping discovery supports named scope/level fields and schema choices expressed with either `enum` or `const`. Two raw ASIN-scoped periods load concurrently. Provider results must match the requested country and exact date range; campaign identity, sponsored type, name, Sales, Spend, and integer Orders are normalized from flat rows, nested campaign/entity records, structured JSON, or the MCP text table paired with structured totals. Rows merge by `sponsoredType:campaignId`, aggregate duplicates within a period, prefer the current name, and retain explicit previous/current activity flags. Missing-period metrics become zero without describing the campaign as paused. Raw MCP content and credentials never cross the server boundary.
