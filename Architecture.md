...
`/ppc/api/dashboard/untargeted-opportunities` is an authenticated, no-store reporting boundary for one selected ASIN and Wednesday reporting week. It caps an in-progress week at yesterday, opens one Scale Insights tool session, and calls `get_search_term_performance` plus `get_ppc_exact_coverage`. The adapter normalizes nested `entity`/`metrics`, structured JSON, and Markdown-table responses. It rejects rows missing any required PPC metric, reduces the result to terms with at least one attributed order, batches those terms through the live `query_list` coverage input, and emits a row only when exact coverage is explicitly false. Missing or ambiguous coverage is excluded.

The dashboard-wide Refresh Data action issues the opportunity request for the active product/week alongside the weekly performance refresh. Validated responses are stored in the capped version-1 `glassco.ppcUntargetedOpportunitiesCache.v1` browser cache and restored by product/week without another MCP call. A refresh token is scoped to the active performance-cache key, so selecting another product does not trigger an implicit opportunity request. Criteria changes, six metric sorts, and filtered aggregate totals remain local. Weighted total ACOS is derived from aggregate Spend and Sales. Row source URLs use the validated Scale Insights Search Terms route with the active ASIN, period, and encoded term; no source URL is persisted. Provider errors expose bounded codes and a correlation ID; logs contain tool property names and counts without search terms, ASIN opportunities, metrics, tokens, or raw response bodies.
