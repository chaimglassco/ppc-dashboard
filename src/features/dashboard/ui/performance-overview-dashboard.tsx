import { BarChart3, CalendarDays, Database, Gauge, PackageSearch } from "lucide-react";
...
function DetailPlaceholder({ section }: { section: (typeof DETAIL_SECTIONS)[number] }) {
  return <section className={styles.ledgerSection}>
    <header><div><span aria-hidden="true" /><div><h2>{section.title}</h2><p>{section.description}</p></div></div><strong>Source pending</strong></header>
    <div className={styles.tableScroll}><table><thead><tr>{section.columns.map(column => <th key={column}>{column}</th>)}</tr></thead><tbody><tr><td colSpan={section.columns.length}><div className={styles.emptyLedger}><Database aria-hidden="true" /><strong>No connected dataset yet</strong><span>This table is ready for a campaign-level Scale Insights or CSV source in a later refinement.</span></div></td></tr></tbody></table></div>
  </section>;
}
...
export function PerformanceOverviewDashboard({ products, reports, currentWeekStart, todayIso }: Props) {
  ...
  const currentSummary = summarize(products, reports, weekStarts.slice(0, 1));
  const fourteenDaySummary = summarize(products, reports, weekStarts.slice(0, 2));
  const thirtyDaySummary = summarize(products, reports, weekStarts);
  const currentRows = products.flatMap(product => {
  ...
  const latestUpdate = Object.values(reports).map(report => report.updatedAt).filter(Boolean).toSorted().at(-1);
  ...
  return <main className={styles.overview} aria-label="Account performance dashboard">
    ...
    <div className={styles.canvas}>
      <header className={styles.overviewHeader}><div><span>Week {getIsoWeekNumber(currentWeekStart)} · Multi-Timeframe Performance Ledger</span><h1>Performance Overview</h1><p>Monochrome account overview modeled from the supplied reference. Weekly cards and ASIN ranking use available local reports.</p></div><div><CalendarDays aria-hidden="true" /><span><small>Audit Window</small><strong>{displayRange(addDaysIso(currentWeekStart, -28), currentWeekEnd)}</strong></span></div></header>

      <section className={styles.temporalSection} aria-labelledby="temporal-ledger-heading">
        <div className={styles.sectionKicker}><span id="temporal-ledger-heading"><i aria-hidden="true" />Temporal Ledger Matrix // Continuous Historical Benchmarks</span><small>Currency: USD ($)</small></div>
      ...
      <section className={styles.ledgerSection}>
        <header><div><span aria-hidden="true" /><div><h2>ASIN Velocity &amp; Performance Ranking // Net Sales Contribution Matrix</h2><p>Product-level results from saved reports for {displayRange(currentWeekStart, currentWeekEnd)}.</p></div></div><strong>{currentRows.length} active row{currentRows.length === 1 ? "" : "s"}</strong></header>
      ...
      <section className={styles.summaryStrip} aria-label="Overview source coverage"><span><BarChart3 aria-hidden="true" /><small>Saved weekly rows</small><strong>{thirtyDaySummary.reports}</strong></span><span><Gauge aria-hidden="true" /><small>30-day PPC ACOS</small><strong>{displayPercent(percentage(thirtyDaySummary.spend, thirtyDaySummary.ppcSales))}</strong></span><span><PackageSearch aria-hidden="true" /><small>Ranked products</small><strong>{currentRows.length}</strong></span></section>

      {DETAIL_SECTIONS.map(section => <DetailPlaceholder key={section.title} section={section} />)}
    </div>
  </main>;
}
