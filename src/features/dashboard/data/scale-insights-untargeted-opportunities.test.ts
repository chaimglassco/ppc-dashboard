...
const definitions = [
  ...
  { name: "get_ppc_exact_coverage", inputSchema: { type: "object", properties: { asin: {}, marketplace: {}, fromDate: {}, toDate: {}, search_terms: {}, limit: {} } } },
] as Tool[];
...
describe("Scale Insights untargeted sales opportunity adapter", () => {
  ...
  it("joins converting queries to explicit uncovered results and classifies product ASINs", async () => {
    const callTool = vi.fn(async (name: string) => name === "get_search_query_data" ? {
      structuredContent: {
        agg: { Currency: "USD" },
        rows: [
          { SearchTerm: "glass cutter", Sales: "$100.00", Orders: "2", Spend: "$20", Clicks: "10" },
          { SearchTerm: "B0ABCDEF12", Sales: 80, Orders: 1, Spend: 8, Clicks: 4, ACOS: "10%" },
          { SearchTerm: "already targeted", Sales: 50, Orders: 1, Spend: 5, Clicks: 2 },
          { SearchTerm: "no sale", Sales: 0, Orders: 0, Spend: 1, Clicks: 1 },
        ],
        oppMeta: { total_count: 4, data_as_of: "2026-09-09" },
      },
    } : {
      structuredContent: {
        uncovered: [{ SearchTerm: "glass cutter" }, { SearchTerm: "B0ABCDEF12" }],
    ...
    expect(callTool).toHaveBeenNthCalledWith(2, "get_ppc_exact_coverage", expect.objectContaining({
      asin: "B012345678", search_terms: ["glass cutter", "B0ABCDEF12", "already targeted"],
    }));
    expect(report).toMatchObject({
      ...
      opportunities: [
        { term: "glass cutter", type: "Search term", sales: 100, orders: 2, spend: 20, clicks: 10, acos: 20 },
        { term: "B0ABCDEF12", type: "Product ASIN", sales: 80, orders: 1, spend: 8, clicks: 4, acos: 10 },
      ],
    });
  });
  ...
  it("reads Markdown tables and never treats unknown coverage as untargeted", async () => {
    const callTool = vi.fn(async (name: string) => ({ content: [{ type: "text", text: name === "get_search_query_data"
      ? "| Search Term | PPC Sales | PPC Orders | PPC Cost | Clicks |\n| --- | --- | --- | --- | --- |\n| lead knife | $25 | 1 | $5 | 3 |"
  ...
  it("does not spend a coverage call when the search report has no converting rows", async () => {
