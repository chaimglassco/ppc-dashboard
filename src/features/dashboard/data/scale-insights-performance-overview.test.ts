...
describe("Scale Insights performance overview adapter", () => {
  it("loads timeframe totals and normalizes campaign, keyword, product-target, and search-term rows", async () => {
    const definitions = [
      tool("get_ads_performance", { group_by: { type: "string", enum: ["campaign"] } }),
      tool("get_sales_data"), tool("get_target_performance"), tool("get_search_term_performance"),
    ];
    const callTool = vi.fn(async (name: string, args: Record<string, unknown>) => {
      if (name === "get_sales_data") return { Summary: { TotalSales: 1000, TotalOrders: 40 } };
      if (name === "get_ads_performance") return {
        ...
        rows: [{ CampaignId: "campaign-1", CampaignName: "Exact Campaign", AdvertisedASIN: "B012345678", TargetingType: "manual", Impressions: 1000, Clicks: 50, Spend: 100, Sales: 500, Orders: 20 }],
      };
