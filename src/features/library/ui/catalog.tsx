...
export function Catalog({ documents }: { documents: LibraryDocument[] }) {
  ...
  return <section className="catalog-panel" aria-label="Library documents">
    ...
    {showDocumentRecovery ? <DeletedDocuments documents={deleted} onClose={() => setShowDocumentRecovery(false)} onRecover={id => persist(current => current.map(document => document.id === id ? { ...document, deletedAt: undefined } : document), "Document recovered.")} /> : null}
