...
export function ManagedReader({ slug, fallback }: { slug: string; fallback?: LibraryDocument }) {
  ...
  useEffect(() => {
    ...
    void hydrateSharedLibraryState(seed, window.localStorage).then(state => {
      if (!cancelled) setDocument(state.documents.find(item => item.slug === slug && !item.deletedAt && !item.hidden) ?? null);
    }).catch(() => {
      if (!cancelled) setDocument(readAdminDocuments(seed, window.localStorage).find(item => item.slug === slug && !item.deletedAt && !item.hidden) ?? null);
    });
