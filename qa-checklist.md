...
- [ ] Every opportunity has a distinct green campaign-creation icon that opens the same selected ASIN/week Search Terms scope and copies the row value. Confirm a keyword row is labeled for `+SKC`, a Product ASIN row is labeled for `+SPC`, and no campaign is created before the operator previews and executes it in Scale Insights.
