...
With adjacent weeks containing data, verify Weekly Performance displays four Sales Metrics cards in the first row, followed by three Order Volume and two Efficiency & Targets cards. Confirm every card emphasizes a large current value, uses a compact percentage badge, and shows the prior value in a `Prev. Week` footer. Lower Spend/ACOS/TACOS must be green; higher sales/orders must be green; movement in the unfavorable direction must be red; unchanged or unavailable comparisons show no badge. Confirm values such as 1667 display as 1,667 while stored precision remains unchanged. Verify the ACOS target-gap copy, TACOS helper copy, centered budget values, and removal of the redundant previous total-sales/TACOS line.
...
- [ ] Open a report with a saved previous week and confirm every Weekly Performance card shows a large current value, an upper-right percentage badge, and a separate `Prev. Week` footer; verify favorable/unfavorable colors follow the metric-specific rules.
