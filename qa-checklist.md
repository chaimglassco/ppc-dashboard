...
- [x] Confirm missing PPC Clicks yields a safe provider warning and unavailable state with no manual click field or substituted search-term/session total.
...
- [ ] Every keyword opportunity has a distinct green campaign-creation icon that opens Scale Insights `/MassCampaigns/KeywordCampaigns/Customize` with the advertised ASIN and exact selected term prefilled. Confirm Product ASIN rows omit this keyword-only action and no campaign is created before the operator previews and executes it in Scale Insights.
