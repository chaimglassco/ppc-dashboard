...
Click Add Goal and verify the selector offers exactly Spend, Sales, PPC Order, Organic Order, and ACOS. Select Organic Order and confirm Number/Percentage appears; Number must equal organic orders, while Percentage must equal rounded organic orders divided by total orders. Verify Actual is read-only, updates when refreshed Scale Insights metrics change, shows Waiting without a snapshot, Partial when the snapshot ends before Tuesday, and Final when it covers Wednesday–Tuesday. Resolve both a Partial and Final goal and confirm Goal History preserves the displayed actual and completeness label. Restore legacy goal data and verify recognized names map safely while unknown names are not discarded.
