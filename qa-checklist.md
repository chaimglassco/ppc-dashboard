...
- [ ] Selecting a product with an ASIN automatically requests the exact Wednesday–Tuesday period and populates Spend, PPC Sales, PPC Orders, Total Sales, and Total Orders from Scale Insights without estimating values.
...
- [ ] Scale Insights OAuth client IDs, secrets, and access tokens are absent from client bundles, network response bodies, browser storage, and browser logs.
