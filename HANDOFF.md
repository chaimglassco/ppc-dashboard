...
- Keyword opportunity rows include a distinct green Scale Insights campaign-creation link. It opens `/MassCampaigns/KeywordCampaigns/Customize` with the advertised ASIN and selected term in the validated `asin` and `keyword` query parameters, landing directly on prefilled Split Keywords to Campaigns setup. Product ASIN rows omit this keyword-only action, and campaign preview/execution remains inside Scale Insights.
