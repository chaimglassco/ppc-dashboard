...
- Repeated production requests on deployment `dpl_GYZJoVmM7pYztV7jnjvHbmmB2ZQL` return a controlled `502` after successful Pipeline session and dashboard requests. The adapter now refuses an aggregate fallback, returns stable missing-capability/unreadable-row codes with a request ID, and emits only sanitized tool/schema and response-shape diagnostics. The UI explains the specific condition and displays the Reference ID.
- The diagnostic change is local and not deployed. It must be reviewed and deployed, then one authenticated Retry must be correlated in Vercel logs before the exact Scale Insights campaign contract is implemented. Do not describe the live campaign table as working until production returns `200` with reconciled real Spend rows.
