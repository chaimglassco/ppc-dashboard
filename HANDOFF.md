...
- The green per-row Scale Insights campaign-creation link was removed. Row checkboxes and the header checkbox select individual or all visible keyword opportunities; Create Bulk Campaigns opens `/MassCampaigns/KeywordCampaigns/Customize` with the advertised ASIN and selected terms as newline-separated values in the validated `asin` and `keyword` query parameters. Product ASIN rows omit keyword selection, and campaign preview/execution remains inside Scale Insights.
...
- Added Good, Bad, and Neutral groups with seven collapsible rule tables. Current-week Spend without Sales and new Spend at 15% ACOS or higher take priority over general Spend/Sales direction rules. Tables compare Spend, Sales, Orders, changes, and current calculated ACOS.
- Added validated capped browser storage under `glassco.ppcCampaignCsvComparison.v1`, scoped by country, selected ASIN, and current Wednesday. Replace CSVs updates the active entry. If the preceding entry's current period exactly matches the selected previous period, its current campaign metrics and filename automatically populate that slot so only the new current CSV is needed. Scale Insights omits report dates and advertised ASIN, so the UI labels exact slots, warns users to export after product filtering, and rejects recognizable filename ranges that conflict with the expected week.
...
- Lint, typecheck, all 344 tests across 56 files (6 skipped), the production build, and `git diff --check` pass. Local browser verification reached the expected Pipeline sign-in boundary with no console errors; authenticated controls are covered by DOM and domain tests.
...
- Conversion is labeled PPC Conversion Rate and uses PPC Orders divided by PPC Clicks. The advertising request first accepts an unambiguous click count from the requested ASIN row or aggregate totals. If unavailable, the shared Refresh Data flow uses the complete Search Term Performance click total for the same ASIN and week without another MCP call. The manual click field is removed; the displayed current/previous rate is a whole percentage while calculations retain two-decimal precision. Total Sessions and incomplete search-term results are not used.
...
## September 16, 2026 — account performance overview

- Dashboard now has a permanent temporal ledger with TACOS below ACOS, applied custom date controls, and centered red/green Spend Share and Sales Share cells. Overview Source Coverage was removed.
- Authenticated no-store `/ppc/api/dashboard/performance-overview` dynamically adapts to the connected Scale Insights tool schemas and returns completed-day totals plus campaign, keyword, product-target, and search-term rows for the selected ASIN/date scope.
- The UI validates the full response, clears stale live rows when scope changes, explains provider/authorization/capability failures, and retains local weekly summaries as a fallback.
- Verification: lint and typecheck pass; all 59 test files pass with 352 tests and 6 skips; production build passes and includes the new dynamic route. Local browser verification reached the expected Pipeline sign-in boundary without console warnings or errors; authenticated production data reconciliation remains part of rollout QA.
