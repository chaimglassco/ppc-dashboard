...
- Keyword opportunity rows include a distinct green Scale Insights campaign-creation link. It opens `/MassCampaigns/KeywordCampaigns/Customize` with the advertised ASIN and selected term in the validated `asin` and `keyword` query parameters, landing directly on prefilled Split Keywords to Campaigns setup. Row checkboxes and the header checkbox select individual or all visible keyword opportunities; Create Bulk Campaigns sends the selected terms as newline-separated values in the same `keyword` parameter. Product ASIN rows omit keyword selection and creation, and campaign preview/execution remains inside Scale Insights.
