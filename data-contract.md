...
The response is accepted only when every field validates. A Product ASIN term must be ten alphanumeric characters. Zero Impressions, Clicks, Spend, Sales, and Orders remain valid; zero Sales yields a null ACOS unless the provider supplies a valid value. The server includes a row only when the exact-coverage source explicitly reports the same normalized term as uncovered. Load Opportunities, Fetch Again, and shared Refresh Data use the same request and response contract. The response is no-store and is not written to `localStorage` or the weekly-report schema. Errors may add `code` and `requestId`; authorization responses may add an allowlisted hosted consent URL.
