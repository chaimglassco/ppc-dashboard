...
The existing version-1 `glassco.ppcPerformanceNotes.v1` report accepts optional goal `metric` (`spend`, `sales`, `ppcOrders`, `organicOrders`, or `acos`) and `unit` (`currency`, `number`, or `percentage`). Unit is canonicalized by metric: Spend/Sales use currency, PPC Order uses number, ACOS uses percentage, and Organic Order accepts number or percentage. Recognizable legacy titles infer a metric; unknown legacy goals remain valid without one. The active goal's stored `actual` remains compatibility data and is not used as an editable source.
