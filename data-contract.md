...
Table sorting, filtered totals, copy controls, and Scale Insights source destinations are derived client-side from this validated response. Total ACOS equals filtered total Spend divided by filtered total Sales; it is unavailable when filtered Sales is zero. Attributed rows open the Campaigns route with the response CampaignId, ASIN, and period. Legacy cached rows without attribution retain the Search Terms route fallback.

The response is accepted only when every field validates. A Product ASIN term must be ten alphanumeric characters. Zero Impressions, Clicks, and Spend remain valid, but every emitted row has integer `orders >= 1`; missing provider metrics invalidate that provider row instead of becoming zero. ACOS is calculated from PPC Spend and Sales when the provider omits it. The server includes a row only when harvest analysis supplies the same normalized term and a nonempty CampaignId. For duplicate mappings, greatest attributed Sales wins, then Orders and Spend. The API response remains no-store and never extends the weekly-report schema.
