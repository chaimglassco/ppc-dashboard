...
- Campaign Week-over-Week Comparison is currently staged as a previous-week Spend baseline. It requests the preceding matched period only when Scale Insights explicitly advertises campaign grouping, lists campaigns by Spend, leaves Current Week Spend blank, and opens the previous-period campaign trend; results remain in memory only. Unsupported or changed provider contracts return correlated safe errors instead of treating ASIN totals as campaign rows.
