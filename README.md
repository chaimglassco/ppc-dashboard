...
The conversion card is now explicitly **PPC Conversion Rate** and calculates `PPC Orders / PPC Clicks`. The Scale Insights request loads one ASIN performance row and accepts exact click fields from that row or the advertising totals without adding another MCP call. The card is provider-driven, has no manual click input, and displays a whole percentage. Missing exact clicks remain unavailable rather than substituting overall sessions or non-reconciling search-term totals.

The selected-product ASIN has an accessible copy button with temporary copied feedback. Weekly-goal ACOS targets also render as whole percentages while their saved target values retain their original precision. Goal History and Add Goal occupy the planning-card header; the active-goal count is removed.
...
- Campaign Week-over-Week Comparison imports two product-filtered Scale Insights Campaign CSVs, joins them by CampaignId, and stores the validated result locally for the selected ASIN/week. Eight collapsible tables group campaigns as Good, Bad, or Neutral using Spend/Sales movement. Spend without Sales in either or both weeks has first priority, followed by lost Sales and newly inefficient Spend at 15% ACOS or higher. Numeric headers sort their own dropdown highest-to-lowest or lowest-to-highest without changing the other situations. Because Scale Insights omits report dates and advertised ASIN from these exports, the UI labels the exact required weeks, rejects recognizable mismatched filename ranges, and tells the operator to export after filtering to the selected product.
...
- `/ppc/api/dashboard/campaign-comparison` applies the authenticated no-store boundary to one previous matched campaign period, refuses aggregate fallback, follows advertised provider pagination, validates scope and Spend, and returns the staged normalized baseline. Sanitized contract diagnostics are correlated by a request ID and exclude provider values and user/account data.
