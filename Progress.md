...
- Added a green campaign-creation icon to every untargeted opportunity. It opens the selected ASIN/week in Scale Insights Search Terms and copies the row value for Instant Search, where keyword rows can start `+SKC` and ASIN rows can start `+SPC` with Scale Insights' normal preview and execute flow.
